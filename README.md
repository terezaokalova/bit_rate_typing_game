...
Before building the game, I ran a Monte Carlo simulation (`simulation.py`) sweeping N from 3 to 40 across stochastic agent profiles under two accuracy models. The linear and logistic models disagreed on N* by roughly a factor of 8 for the same agent. Seeing that gap, I stopped trying to pick N based on theoretical modeling alone and built a calibration system so that I could measure each player's accuracy curve.
...
All runs below are 60-second scored runs unless noted. Shorter sandbox runs at 15s reached higher peak rates (13.31 at N=26, 14.26 at N=28, 14.41 at N=27, **15.79 at N=26**) but are not directly comparable to the grading duration and are reported here only as ceiling estimates.

| Run | N | Sc | Si | Accuracy | Bit Rate | Notes |
|-----|---|----|----|----------|----------|-------|
| 1 | 10 | 69 | 0 | 100% | 3.65 | No preview, first attempt |
| 2 | 10 | 114 | 4 | 96.6% | 5.81 | Lookahead added |
| 3 | 10 | 119 | 3 | 97.5% | 6.13 | |
| 4 | 10 | 137 | 1 | 99.3% | 7.19 | |
| 5 | 26 | 163 | 0 | 100% | 12.62 | First calibrated run, 26 letters |
| 6 | 31 | 77 | 3 | 96.3% | 12.10 | 30-second run |

My 15-second sandbox runs reached 13.31–15.79 bps, suggesting a sustained 60-second ceiling near 13–14 bps once fatigue is factored in.

### First-session testing

Nine users completed either the data collection protocol (6 rounds at varying N) or a scored run. None had prior exposure to the game. Test participants spanned a wide range of typing speeds (~330–830ms mean RT) and optimal N values (18–31), confirming the adaptive calibration design.

| User | Best N | Best bps | Accuracy | Profile |
|------|--------|----------|----------|---------|
| 1 | 30 | 6.15 | 100% | Slow, flawless (~700ms RT) |
| 2 | 24 | 5.88 | 100% | Moderate, peaks mid-range |
| 3 | 24 | 6.94 | 96.0% | Fast, non-monotonic accuracy |
| 4 (session 1) | 30 | 7.93 | 96.2% | Slow-accurate, first exposure |
| 4 (session 2) | 30 | 8.58 | 98.2% | Practice effect (+8% across sessions) |
| 5 | 18 | 5.86 | 100% | Very slow (~800ms RT), low-N optimum |
| 6 | 29 | 7.69 | 78.6% | Fast typist, accuracy collapse at large N |
| 7 | 31 | 8.59 | 100% | Slow-flawless, highest Hick slope (b=0.126) |
| 8 | 31 (scored) | 4.25 | 86.1% | Calibration failure case, phase-shift cascade |
| 9 | 24 | 13.27 | 96.8% | Fast gamer, strong across all N |

**User 9** reached notable performance. In a single session with no prior exposure, their performance progressed through three phases: (1) a 6-round calibration sweep peaking at 13.27 bps at N=24 (collect protocol), (2) a full 60-second scored run at N=30 reaching 14.65 bps (182 correct, 1 error, 99.45% accuracy), and (3) two 60-second runs at N=32 (full alphabet, calibration skipped) reaching 14.65 and **16.27 bps** (202 correct, 5 errors, 97.58% accuracy, 95.17% Shenoy efficiency). Three subsequent 30-second sandbox runs at N=32 hit **16.51 bps** (100/0), **17.01 bps** (103/0), and **17.34 bps** (105/0), all at 100% accuracy, suggesting headroom above the 60-second figure.

Optimal N varied from 18 to 31, spanning most of the available range. No single fixed N would have been optimal for all players, validating the adaptive design. User 7 was a slow but flawless player with a fitted Hick-Hyman slope b=0.126 (the highest observed), confirming that larger alphabets genuinely slow this player while faster typists showed slopes near zero. User 5 was the slowest overall (~800ms RT) and peaked at N=18, the lowest optimum in the dataset. User 3 showed a non-monotonic accuracy curve (98.2% at N=10, 86.5% at N=18, 96% at N=24), which is exactly the case the linear model p = p₀ - c·log₂(N) fails on. The N* sweep still identified N=24 correctly because it evaluates all N values empirically, not just the linear prediction.
